...
def generate_previous_motion(speed, throttle, steer, brake):
    """
    generate the previous motion using motion para
    """
    speed = format_number(speed)
    throttle = format_number(throttle)
    steer = format_number_steer(steer)
    motion_status = f"The car is moving at a speed of {speed} m/s, with a steering angle of {steer} degrees and a throttle setting of {throttle}."
    if brake:
        brake_status = "The car is applying brakes. "
    else:
        brake_status = " "
    previous_motion = motion_status + brake_status
...
class ADHAgent(autonomous_agent.AutonomousAgent):
    def setup(self, path_to_conf_file):
        ...
        disable_torch_init()
        model_name = get_model_name_from_path(self.config.planner_model_ckpt)
        self.planner_tokenizer, self.planner, self.planner_image_processor, self.context_len = load_pretrained_model(
            self.config.planner_model_ckpt, None, model_name, load_8bit=self.config.planner_load_8bit, load_4bit=self.config.planner_load_4bit, device=self.config.planner_device
        )
        self.setup_conversation_mode(model_name)
        print('Planner model loaded.')
        
        
        print('Loading controller model...')
        model_cls = registry.get_model_class('drive_agent')
        self.controller = model_cls(preception_model=self.config.controller_preception_model,
                          preception_model_ckpt=self.config.controller_preception_model_ckpt,
                          llm_model=self.config.controller_model,
                          max_txt_len=64,
                          use_notice_prompt=self.config.agent_use_notice,
                          has_motion=self.config.has_motion,
                          has_llava_vision_tower=self.config.has_llava_vision_tower
                          )
        self.controller.load_state_dict(torch.load(self.config.controller_model_ckpt)["model"], strict=False)
        self.controller.to(self.config.controller_device)
        ...
        # create the initial motion
        if self.config.use_previous_motion:
            os.environ["PRE_MOTION"] = "The car is moving at a speed of 0 m/s, with a steering angle of 0 degrees and a throttle setting of 0. The car is applying brakes. " 
    ...
    def setup_conversation_mode(self, model_name):
    ...
    def run_step(self, input_data, timestamp):
        ...
        image_embeds = self.controller.visual_encoder(input_data)
        ...
        planner_prompt=f"What motion should the car currently take to accomplish the instruction '{input_data['text_input'][0]}'?"
        # print("the instruction is {}".format(input_data['text_input'][0]))

        if self.config.use_previous_motion:
            previous_motion = os.getenv("PRE_MOTION")
            planner_prompt = previous_motion + planner_prompt
            
        # rgb_front, rgb_left, rgb_right, rgb_rear上下合并成一张图片

        pil_transforms=transforms.ToPILImage()
        rgb_front=pil_transforms(rgb_front[0].to(self.config.planner_device))
        rgb_left=pil_transforms(rgb_left[0].to(self.config.planner_device))
        rgb_right=pil_transforms(rgb_right[0].to(self.config.planner_device))
        rgb_rear=pil_transforms(rgb_rear[0].to(self.config.planner_device))
        merged_image = Image.new('RGB', (rgb_front.size[0], rgb_front.size[1] * 4))
        merged_image.paste(rgb_front, (0, 0))
        merged_image.paste(rgb_left, (0, rgb_front.height))
        merged_image.paste(rgb_right, (0, rgb_front.height * 2))
        merged_image.paste(rgb_rear, (0, rgb_front.height * 3))
            
        image_tensor = process_images([merged_image], self.planner_image_processor, self.planner.config)
        if type(image_tensor) is list:
            image_tensor = [image.to(self.config.planner_device, dtype=torch.float16) for image in image_tensor]
        else:
            image_tensor = image_tensor.to(self.config.planner_device, dtype=torch.float16)
        # Building prompt with default image tokens
        full_prompt = DEFAULT_IM_START_TOKEN + DEFAULT_IMAGE_TOKEN + DEFAULT_IM_END_TOKEN + '\n' + planner_prompt
        self.conv.append_message(self.conv.roles[0], full_prompt)
        input_ids = tokenizer_image_token(full_prompt, self.planner_tokenizer, IMAGE_TOKEN_INDEX, return_tensors='pt').unsqueeze(0).to(self.config.planner_device)
        
        with torch.inference_mode():
            output_ids = self.planner.generate(
                input_ids,
                images=image_tensor,
                image_sizes=[merged_image.size],
                do_sample=True,
                temperature=0.2,
                max_new_tokens=512,
                use_cache=True
            )
        planner_motion = self.planner_tokenizer.decode(output_ids[0]).strip()
        planner_motion = re.search(r'<s>(.*?)</s>', planner_motion)

        input_data['motion'] = [planner_motion.group(1) if planner_motion else "None"]
        # print("the motion!!!!!!!!!!!! is {}".format(input_data['motion'][0]))

        with torch.cuda.amp.autocast(enabled=True):
            waypoints, is_end = self.controller(input_data, inference_mode=True, image_embeds=image_embeds)
        ...
        # update the prevouos motion
        if self.config.use_previous_motion:
            os.environ["PRE_MOTION"] = generate_previous_motion(metadata["speed"], throttle, steer, brake)
    ...
    def destroy(self):
        del self.planner_tokenizer
        del self.planner
        del self.planner_image_processor
        del self.controller
